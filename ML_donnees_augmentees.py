...
#%% Pour les données augmentéesµ# Créer un dictionnaire qui mappe le code vers la valeur (df['value'])

df_unique = df.drop_duplicates(subset=['code'])
code_to_value = dict(zip(df_unique['code'], df_unique['value']))
print(code_to_value)

base_dir = "./data/" 
augmented_data_dir = os.path.join(base_dir, "donnees_augmentees") 
...
def save_augmented_images(file_paths, labels):
    ...
    for idx, file_path in enumerate(file_paths):
        ...
        for aug_idx, aug_img in enumerate(augmented_images):
            ...
            aug_file_name = f"aug_{idx}_{aug_idx}_{value}.jpg"
            ...
            if not os.path.exists(aug_file_path):
                ...
                if not images_existing:
                    print(f"Certaines images augmentées existent déjà et ne seront pas recréées.")
                    images_existing = True
...
print("Classes présentes dans y_train_augmented :", np.unique(y_train_augmented))

print(f"Nombre d'images originales : {len(df['file_path'])}")
print(f"Nombre total d'images après augmentation : {len(X_train_augmented)}")
...
# Exemple : Afficher les 5 premières images augmentées
...
ada_model.fit(X_train_pca, y_train_augmented)
y_pred_ada = ada_model.predict(X_test_pca)
ada_accuracy = accuracy_score(y_test_augmented, y_pred_ada)
print(f"AdaBoost Accuracy on Augmented Data: {ada_accuracy:.4f}")

#%% Gradient Boosting
from sklearn.ensemble import GradientBoostingClassifier
gb_model = GradientBoostingClassifier(n_estimators=100, random_state=42)
gb_model.fit(X_train_pca, y_train_augmented)
y_pred_gb = gb_model.predict(X_test_pca)
gb_accuracy = accuracy_score(y_test_augmented, y_pred_gb)
print(f"Gradient Boosting Accuracy on Augmented Data: {gb_accuracy:.4f}")

#%% XGBoost
import xgboost as xgb
dtrain = xgb.DMatrix(X_train_pca, label=y_train_augmented)
dtest = xgb.DMatrix(X_test_pca, label=y_test_augmented)
...
xgb_accuracy = accuracy_score(y_test_augmented, y_pred_xgb)
print(f"XGBoost Accuracy on Augmented Data: {xgb_accuracy:.4f}")
